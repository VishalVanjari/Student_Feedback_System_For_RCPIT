...
if (!empty($_POST)) {
    ...
    // Fetch Name
    $nameQuery = "SELECT * FROM faculty WHERE faculty_id=? or department ='Admin User'";
    ...
    // Fetch Subject and Year based on Faculty ID, Semester, and Academic Year
    $query = "SELECT subject, year FROM faculty WHERE faculty_id = ? AND semester = ? AND academic_year = ?";
    $stmt = mysqli_prepare($al, $query);
    mysqli_stmt_bind_param($stmt, "sss", $faculty_id, $semester, $academic_year);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $row = mysqli_fetch_array($result);

    $subject = $row['subject'];
    $faculty_year = $row['year'];
    ...
    $sql = "SELECT * FROM feeds WHERE faculty_id=? AND year=? AND division=? AND academic_year=? AND semester=?";
    $sqlStmt = mysqli_prepare($al, $sql);
    mysqli_stmt_bind_param($sqlStmt, "sssss", $faculty_id, $year, $division,$academic_year,$semester);
    ...
    if ($totalRows == 0) {
        ...
        echo "<script>alert('Feedback doesn't exist.'); window.location.href = 'specific_feeds.php';</script>";
...
<div class="report-head">
    ...
    <h3 style = "font-weight:bold;color:black;margin-top:-10px;margin-bottom:-10px;">Specific Feedback</h3><br>
<table>
<tr>
        <td>
            <?php
        ...
        <td class="table-faculty-details" style="font-weight:bold; text-align:left;" colspan="9">
            ...
            Year : <?php echo htmlspecialchars($faculty_year); ?><br>
...
<tr>
    ...
    <td style="font-weight:bold;">
        ...
        <br>
        <br>
        <input type="button" class= "button1" onClick="window.print();" value="PRINT">&nbsp;
        <input type="button" class= "button1" onClick="window.location='specific_feeds.php'" value="BACK">
